...
export class Debug extends Viewlet {
    ...
    async openDebugConsole(): Promise<void> {
        if (process.platform === 'darwin') {
            await this.code.dispatchKeybinding('cmd+shift+y');
        } else {
            await this.code.dispatchKeybinding('ctrl+shift+y');
        }
    ...
    async startDebuggingNode(): Promise<number> {
    ...
    public async debuggerHasStarted() {
        const ele = await this.code.waitForElement('div.debug-toolbar');
        assert.equal(ele.attributes['aria-hidden'], undefined);
        // await this.code.waitForElement(PAUSE);
        // await this.code.waitForElement(DEBUG_STATUS_BAR);
    }
    async startDebugging(waitForText: string): Promise<void> {
        ...
        await this.code.waitForElement(DEBUG_STATUS_BAR);
        // const portPrefix = 'Port: ';
        await this.openDebugConsole();
        await this.waitForOutput(output => {
            return output.some(line => line.indexOf(waitForText) >= 0);
        });
        // const lastOutput = output.filter(line => line.indexOf(portPrefix) >= 0)[0];
