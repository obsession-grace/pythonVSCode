...
export class Editor {
    ...
    async gotoDefinitionWithF12(filename: string, term: string, line: number): Promise<void> {
        await this.clickOnTerm(filename, term, line);
        await this.code.dispatchKeybinding('F12');
    }
    ...
    public async clickOnTerm(filename: string, term: string, line: number): Promise<void> {
