...
export class Problems {
    ...
    public static getSelectorForProblemCount(): string {
        return `.part.panel.bottom .monaco-action-bar .action-item .badge[title*='Problems'] div.badge-content`;
    }

    public static getSelectorInProblemsViewForMessage(problemType: ProblemSeverity): string {
        let selector = problemType === ProblemSeverity.WARNING ? 'warning' : 'error';
        return `div[id="workbench.panel.markers"] .monaco-tl-contents .marker-icon.${selector} + div + div > div > div > span > span`;
    }
