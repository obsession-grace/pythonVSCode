...
export function setup() {
    describe('Debug', () => {
        it('configure launch json', async function() {
            ...
            await app.workbench.editor.waitForEditorContents('launch.json', contents =>
                /"protocol": "inspector"/.test(contents)
            );
        ...
        it('start debugging', async function() {
            ...
            port = await app.workbench.debug.startDebuggingNode();
        ...
        it('stepOver, stepIn, stepOut', async function() {
            ...
            const first = await app.workbench.debug.waitForStackFrame(
                sf => /response\.js$/.test(sf.name),
                'looking for response.js'
            );
